...
struct wlr_linux_drm_syncobj_surface_v1_commit {
	...
	struct wlr_linux_drm_syncobj_surface_v1 *surface_sync;
	uint32_t cached_seq;
};

static const struct wp_linux_drm_syncobj_v1_interface explicit_sync_impl;
static const struct wp_linux_drm_syncobj_timeline_v1_interface sync_timeline_impl;
static const struct wp_linux_drm_syncobj_surface_v1_interface surface_sync_impl;

static struct wlr_linux_drm_syncobj_v1 *explicit_sync_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&wp_linux_drm_syncobj_v1_interface, &explicit_sync_impl));
...
static struct wlr_render_timeline *sync_timeline_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&wp_linux_drm_syncobj_timeline_v1_interface, &sync_timeline_impl));
	return wl_resource_get_user_data(resource);
}

// Returns NULL if the surface sync is inert
static struct wlr_linux_drm_syncobj_surface_v1 *surface_sync_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&wp_linux_drm_syncobj_surface_v1_interface, &surface_sync_impl));
	return wl_resource_get_user_data(resource);
}
...
static struct wlr_linux_drm_syncobj_surface_v1 *surface_sync_from_surface(
...
static void surface_sync_handle_surface_client_commit(struct wl_listener *listener,
	...
	struct wlr_linux_drm_syncobj_surface_v1_state pending = surface_sync->pending;
	surface_sync->pending = (struct wlr_linux_drm_syncobj_surface_v1_state){0};
...
static void explicit_sync_handle_get_surface(struct wl_client *client,
	...
	if (surface_sync_from_surface(explicit_sync, surface) != NULL) {
		wl_resource_post_error(resource,
			WP_LINUX_DRM_SYNCOBJ_V1_ERROR_SURFACE_EXISTS,
			"wp_linux_drm_syncobj_surface_v1 already created for this surface");
		return;
	}

	struct wlr_linux_drm_syncobj_surface_v1 *surface_sync = calloc(1, sizeof(*surface_sync));
	if (surface_sync == NULL) {
...
static void explicit_sync_handle_import_timeline(struct wl_client *client,
	...
	if (timeline == NULL) {
		wl_resource_post_error(resource,
			WP_LINUX_DRM_SYNCOBJ_V1_ERROR_INVALID_TIMELINE,
...
static void explicit_sync_bind(struct wl_client *client, void *data,
	...
	struct wlr_linux_drm_syncobj_v1 *explicit_sync = data;

	struct wl_resource *resource = wl_resource_create(client,
		&wp_linux_drm_syncobj_v1_interface, version, id);
	if (resource == NULL) {
...
struct wlr_linux_drm_syncobj_v1 *wlr_linux_drm_syncobj_v1_create(
	...
	explicit_sync->global = wl_global_create(display,
		&wp_linux_drm_syncobj_v1_interface, LINUX_DRM_SYNCOBJ_V1_VERSION,

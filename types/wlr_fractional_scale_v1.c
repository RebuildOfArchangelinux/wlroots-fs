...
static void fractional_scale_global_handle_get_fractional_scale(
	...
	scale->resource = wl_resource_create(client,
...
static const struct wp_fractional_scale_global_interface fractional_scale_global_impl = {
	.destroy = resource_handle_destroy,
	.get_fractional_scale = fractional_scale_global_handle_get_fractional_scale,
};

static void global_bind(struct wl_client *client, void *data,
		uint32_t version, uint32_t id) {
	struct wlr_fractional_scale_global *global = data;

	struct wl_resource *resource = wl_resource_create(client,
		&wp_fractional_scale_global_interface, version, id);
	if (resource == NULL) {
		wl_client_post_no_memory(client);
		return;
	}
	wl_resource_set_implementation(resource,
		&fractional_scale_global_impl, global, NULL);
}

static void fractional_scale_global_handle_display_destroy(
...
struct wlr_fractional_scale_global *wlr_fractional_scale_global_create(
	...
	struct wlr_fractional_scale_global *global = calloc(1, sizeof(*global));
	if (global == NULL) {
		return NULL;
	}

	global->global = wl_global_create(display, &wp_fractional_scale_global_interface,

...
static void rules_to_logical(struct wlr_xdg_positioner_rules *rules,
		double factor) {
...
struct wlr_xdg_popup_configure *send_xdg_popup_configure(
	...
	*configure = popup->scheduled;
	...
	xdg_popup_send_configure(popup->resource,
		round(geometry->x * factor),
		round(geometry->y * factor),
		round(geometry->width * factor),
		round(geometry->height * factor));
...
static void xdg_popup_handle_reposition(
	...
	rules_to_logical(&popup->scheduled.rules,
...
void create_xdg_popup(struct wlr_xdg_surface *surface,
	...
	rules_to_logical(&surface->popup->scheduled.rules,
		surface->surface->client_scale_factor);
	wlr_xdg_positioner_rules_get_geometry(
		&surface->popup->scheduled.rules,

...
static void render_pass_add_rect(struct wlr_render_pass *wlr_pass,
	...
	case WLR_RENDER_BLEND_MODE_PREMULTIPLIED:;
		...
		wlr_matrix_project_fbox(matrix, &box, WL_OUTPUT_TRANSFORM_NORMAL, 0, proj);
...
static void render_pass_add_texture(struct wlr_render_pass *wlr_pass,
	...
	float proj[9], matrix[9];
	wlr_matrix_identity(proj);
	wlr_matrix_project_fbox(matrix, &dst_box, options->transform, 0, proj);
	wlr_matrix_multiply(matrix, pass->projection, matrix);
	...
	for (int i = 0; i < clip_rects_len; i++) {
		...
		struct wlr_fbox intersection;
		if (!wlr_fbox_intersection(&intersection, &dst_box, &clip_box)) {
			continue;
		}
		render_pass_mark_box_updated(pass, &intersection);
	}

...
struct server {
	...
	struct wlr_linux_drm_syncobj_v1 *drm_syncobj_v1;
...
static void output_handle_frame(struct wl_listener *listener, void *data) {
	...
	wl_list_for_each(surface, &output->server->surfaces, link) {
		...
		struct wlr_linux_drm_syncobj_surface_v1_state *sync_v2_state =
			...
			output->server->drm_syncobj_v1, surface->wlr);
...
int main(int argc, char *argv[]) {
	...
	int drm_fd = wlr_renderer_get_drm_fd(server.renderer);
	server.drm_syncobj_v1 =
		wlr_linux_drm_syncobj_v1_create(server.display, drm_fd);
